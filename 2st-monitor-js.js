...
const CONFIG = {
  ...
  // プロキシ設定（複数対応）
  proxies: (process.env.PROXY_LIST || 'http://162.43.73.18:8080').split(','),
...
class StatsManager {
  ...
  async update(newItemCount) {
    ...
    this.stats.hourlyNewItems[currentHour] += newItemCount;
  ...
  async scrapeURL(page, urlConfig) {
    ...
    try {
      ...
      try {
        ...
        console.log(`    ⚠️  トップページ取得失敗: ${error.message}`);
      }
    ...
    for (const urlConfig of URLS) {
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          if (error.message === '403_FORBIDDEN' && retryCount < maxRetries) {
            console.log(`    🔄 403エラー - プロキシローテーション実行`);
            browser.rotateProxy();
            await browser.close();
            await randomDelay(5000, 10000);
            const newPage = await browser.launch();
